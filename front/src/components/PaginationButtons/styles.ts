...
export const PaginationButton = styled.button<{$highlighted: boolean}>`
    ...
    background-color: ${({ $highlighted }) => ($highlighted ? "#C12a23" : "#FFFFFF")};
    font-weight: 400;
    font-size: 16px;
    color:  ${({ $highlighted }) => ($highlighted ? "#Ffffff" : "#000000")};


    &:hover {
        cursor: pointer;
        background-color: ${({ $highlighted }) => ($highlighted ? "#C12a23" : "#e0e0e0")};
    }
